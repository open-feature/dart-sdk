...
| Tracking | 2.7.1; 6.1.1.1; 6.1.3-6.1.4; 6.2.1-6.2.2 (experimental) | Implemented nonblocking `trackEvent`, optional tracking capability/no-op, numeric value and opt-in immutable custom details. Tracking uses API/transaction/client/invocation precedence and contains transport failures. | Full report and version review remain #165; `value` widens from double? to num?, requiring migration for readers assigning directly to double?. Static-context 6.1.2.1 is not applicable to this package. | `tracking_transaction_contract_test.dart`: integer/double/custom fields, targeting precedence, nonblocking failure isolation, no-tracking, legacy identity and provider-owned drain. | Awaitable `track` and legacy const attribute semantics remain. Strict validation is opt-in; SDK does not impose a transport or delivery guarantee. See tracking-and-transactions.md. |
| Transaction context | 3.3.1.1; 3.3.1.2.1-3.3.1.2.3 (experimental) | Implemented API carrier registration/replacement/removal and scoped canonical context; default zone carrier isolates nested/concurrent async work. API shutdown invalidates scopes and removes registration. | Full report remains #165. Static-context prohibition 3.3.2.1 does not apply to this dynamic-context SDK. Custom carriers must be request-local and independently owned. | `tracking_transaction_contract_test.dart`: custom/default carriers, snapshot, nested failure restoration, concurrency, legacy duplicate IDs, precedence, missing carrier, replacement/removal, shutdown and peer isolation. | Existing TransactionContextManager remains the fallback when no explicit carrier is registered. Carrier types are in experimental/transaction_context.dart. |
